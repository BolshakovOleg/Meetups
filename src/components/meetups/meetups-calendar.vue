    ...
    <div class="rangepicker__calendar">
      ...
      <div class="rangepicker__date-grid">
        <div
          ...
          :class="[
            'rangepicker__cell',
            { rangepicker__cell_inactive: !cell.active }
          ]"
        >
          {{ cell.day }}
          <a
...
export default {
  ...
  computed: {
    ...
    cells() {
      ...
      const firstDayOfMonth = this.startFromSunday
        ? dayOfWeek + 1
        : dayOfWeek || 7;
      const daysInMonth = this.getDaysInMonth(this.date);
      const multiplier = Math.ceil(
        (firstDayOfMonth - (this.offset - 1) + daysInMonth) / 7
      );
      const loop = multiplier * 7;
      const startDate = new Date(
        this.date.getFullYear(),
        this.date.getMonth(),
        this.offset - firstDayOfMonth
      );
